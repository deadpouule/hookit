...
library ProtocolConstants {
    ...
    /// @dev When the live ETH/USD feed is unavailable the factories fall back to the last synced
    ///      snapshot, but only while it is younger than this (a reverting feed must not let a caller
    ///      pick between an arbitrarily old snapshot and the live price).
    uint256 internal constant USD_SNAPSHOT_MAX_AGE = 24 hours;
    ...
    uint32 internal constant MIN_HOLDER_AIRDROP_EPOCH_SECONDS = 60;
    uint32 internal constant MAX_HOLDER_AIRDROP_EPOCH_SECONDS = 7 days;
    ...
    /// @dev Default: consuming 100% of in-range quote depth hits max fee.
    uint16 internal constant DYNAMIC_FEE_DEFAULT_DEPTH_SATURATION_BPS = 10_000;

    /// @dev Fixed fully-diluted valuation at launch ($5,000 with 18-decimal USD scale).
    uint256 internal constant TARGET_LAUNCH_MCAP_USD_X18 = 5_000e18;

    /// @dev Default ETH/USD used to convert the $5k FDV into quote (ETH) at launch.
