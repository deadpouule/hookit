...
export async function GET() {
  try {
    ...
    const [ethUsd, launchEthUsd] = await Promise.all([readEthUsd(client), readLaunchEthUsd(client)]);
    return Response.json({ ethUsd, launchEthUsd, factory: getLaunchFactoryAddress() ?? null });
  } catch (err) {
