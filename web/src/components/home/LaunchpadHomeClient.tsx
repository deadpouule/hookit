...
const LeanHero = dynamic(
  () => import("@/components/home/market/LeanHero").then((m) => m.LeanHero),
  {
    loading: () => (
      <div
        className="lean-hero min-h-[10rem] animate-pulse rounded-2xl bg-zinc-900/40 phone:min-h-[8rem]"
        aria-hidden
      />
    ),
  },
...
export function LaunchpadHomeClient({ initialPools = [] }: { initialPools?: TokenPool[] }) {
  return (
    <div className="min-h-dvh bg-background pb-40 phone:pb-28">
      <SiteHeader />
      <div className="market-shell space-y-6 pt-4 desk:space-y-3 desk:pt-3 phone:space-y-4 phone:pt-3">
        <LeanHero />
