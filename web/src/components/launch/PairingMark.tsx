...
function Wrap({ children, fill }: { children: ReactNode; fill: string }) {
  return (
    <span className="pick-logo" style={{ background: fill }}>
      {children}
    </span>
  );
}

export function PairingMark({ id }: { id: PairingTokenId }) {
  switch (id) {
    case "eth":
      return (
        <Wrap fill="#627eea">
